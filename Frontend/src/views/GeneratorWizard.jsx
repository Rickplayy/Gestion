...
function GeneratorWizard({ carreras, onMessage, onBack }) {
  ...
  const handleCreateTerm = async (e) => {
    ...
    try {
      ...
      setTermId(term.id);
      onMessage({ text: `Ciclo escolar "${term.descripcion}" creado (id ${term.id}).`, type: 'success' });
    } catch (err) {
      onMessage({ text: `Error creando el ciclo: ${err.message}`, type: 'error' });
    } finally {
  ...
  const handleSecuenciasSelect = async (e) => {
    ...
    try {
      ...
      onMessage({ text: `Error procesando secuencias: ${err.message}`, type: 'error' });
    } finally {
  ...
  const handleAspirantesSelect = async (e) => {
    ...
    try {
      ...
      onMessage({ text: `Error procesando aspirantes: ${err.message}`, type: 'error' });
    } finally {
  ...
  const handleConteo = async () => {
    ...
    try {
      ...
      onMessage({ text: `Error obteniendo el conteo: ${err.message}`, type: 'error' });
    } finally {
  ...
  const handleAsignarYExportar = async () => {
    ...
    if (target?.cancelled) {
      onMessage({ text: 'Guardado cancelado.', type: 'error' });
      return;
    }
    ...
    try {
      ...
      onMessage({ text: `Error asignando/exportando: ${err.message}`, type: 'error' });
    } finally {
      ...
      <section className={`step-card ${termId ? 'is-done' : ''}`}>
        <div className="step-head">
          ...
          <div className="step-title">
            <h3>Ciclo escolar</h3>
            <p className="muted">Crea el ciclo escolar en el que vas a registrar los grupos y aspirantes.</p>
          </div>
        </div>
